...
These scripts are intended to be run by me (Katie) in their current format, but can be adapted for other users.  
...
- It is assumed that the reference source catalog ATLAS-Refcat2 is saved to disk in `/etc/rico/atlas_refcat2/` in a certain format (see `select_cat_sample.py` for a description of that format).  
